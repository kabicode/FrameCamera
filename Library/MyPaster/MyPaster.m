...
@interface MyPaster () <MyPasterItemProtocol>
...
- (void)saveAllPasterParameters {
    ...
    [_contentImageView.subviews enumerateObjectsUsingBlock:^(__kindof UIView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if ([obj isKindOfClass:[MyPasterItem class]])
        {
            ...
            [tpItem saveParametersIntoPaster];
        }
    }];
}
...
- (void)didTapDeleteButtonWith:(Paster *)paster {
    if (self.delegate && [self.delegate respondsToSelector:@selector(myPaster:deletePaster:)]) {
        [self.delegate myPaster:self deletePaster:paster];
    }
}
...
-(id)init
{
    ...
    if (self)
    {
        self.paster_id = [[NSDate date] timeIntervalSince1970];
    }
    return self;
}
...
- (BOOL)isEqual:(id)object {
    if ([object isKindOfClass:[Paster class]]) {
        return [(Paster *)object paster_id] == self.paster_id;
    }
