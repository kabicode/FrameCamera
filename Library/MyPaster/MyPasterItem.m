...
{
    if (self.delegate && [self.delegate respondsToSelector:@selector(didTapDeleteButtonWith:)]) {
        [self.delegate didTapDeleteButtonWith:self.paster];
    }
...
-(void)rotateTapGes:(UITapGestureRecognizer *)ges
{
    if (ges.state == UIGestureRecognizerStateEnded)
    {
        ...
        [self setContentViewMirror:haveTransform];
    }
}
...
-(void)rotationGes:(UIRotationGestureRecognizer *)ges
{
    ...
    ges.rotation = 0 ;
    NSLog(@"rotate angle --- %f", [[self.layer valueForKeyPath:@"transform.rotation"] floatValue]);
}
